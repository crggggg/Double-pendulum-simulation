...
// Normalized parameters for fast fractal generation
...
// 3000 steps (Reduced from 9000)
const SIM_STEPS = 3000;
const DT = 0.01; 
const EPSILON = 0.001;

const ChaosFractalVisualizer: React.FC<Props> = ({ simulation }) => {
  ...
  useEffect(() => {
    ...
    const processBatch = () => {
      ...
      while (pixelIndex < totalPixels && (performance.now() - startTime) < MAX_TIME_PER_FRAME) {
        ...
        const theta1 = BOUNDS.minTheta + (x / RESOLUTION) * BOUNDS.range;
        const theta2 = BOUNDS.minTheta + (y / RESOLUTION) * BOUNDS.range;

        // Start with 0 velocity
        let s1: PendulumState = { theta1, theta2, omega1: 0, omega2: 0 };
        let s2: PendulumState = { theta1: theta1 + EPSILON, theta2: theta2 + EPSILON, omega1: 0, omega2: 0 };
        ...
        // Non-linear mapping to show structure in dark areas
  ...
  const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    const newScale = Math.max(1, Math.min(transform.scale * factor, 50)); // Min scale 1 to prevent zoom out
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (isDragging) return; // Prevent click on drag release
    ...
    const transformedY = (my - transform.y) / transform.scale;

    const imageX = transformedX - offsetX;
    const imageY = transformedY - offsetY;
    ...
    const theta1 = -Math.PI + pctX * (2 * Math.PI);
    const theta2 = -Math.PI + pctY * (2 * Math.PI);

    // Reset with 0 velocity
    simulation.reset(theta1, theta2, 0, 0);
  };
  ...
  const handleDownload = (e: React.MouseEvent) => {
    ...
    if (bufferCanvasRef.current) {
        ...
        link.download = `angle-fractal.png`;
  ...
  return (
    <div 
      ...
      title="Angle Fractal: Scroll to Zoom, Drag to Pan, Click to Simulate (v=0)"
    >
      ...
      <div className="absolute bottom-4 right-4 flex gap-2">
        ...
        <button onClick={handleDownload} className="bg-cyan-900/50 hover:bg-cyan-800/80 text-cyan-100 text-xs px-2 py-1 rounded border border-cyan-800 font-mono">
            SAVE PNG
        </button>
      </div>
      ...
      {isProcessing && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm z-10 pointer-events-none">
          <div className="text-cyan-400 font-mono text-sm mb-2">GENERATING ANGLE MAP...</div>
          <div className="w-48 h-2 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-cyan-500 transition-all duration-75 ease-linear" style={{ width: `${progress}%` }} />
          </div>
...
export default ChaosFractalVisualizer;